...
@section('content')
    <div class="">
        ...
        <div class="w-full overflow-hidden md:flex justify-between pb-10">
            @if ($previousVideo)
                <a class="mb-2 no-underline flex items-center md:w-1/2 md:pr-4 text-sm"
                   href="{{ $previousVideo->url }}">
            ...
            @endif
            @if ($nextVideo)
                <a class="mb-2 no-underline flex items-center md:justify-end ml-auto text-sm"
                   href="{{ $nextVideo->url }}">
        ...
        >
            <iframe class="absolute pin w-full h-full"
                    src="https://player.vimeo.com/video/{{ $currentVideo->vimeo_id }}?loop=false&amp;byline=false&amp;portrait=false&amp;title=false&amp;speed=true&amp;transparent=0&amp;gesture=media&amp;color=f9933a"
        ...
        <div class="text-lg markup-lists-alt markup-lists-spaced markup-code">
